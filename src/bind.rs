    ...
    async fn bind(&mut self,target_ip:&str, target_port:u16) -> Result<(), SocksError>
    {
        match self {
            Client::Socks4(inner) | Client::Socks4a(inner) => {
                ...
                let target = target_ip.parse::<Ipv4Addr>()
                .map_err(|e| SocksError::BindError(e.to_string()))?;
                let mut request = vec![0x04, SOCKS_CMD_BIND];
                ...
                {
                    let mut proxy_ip = stream.peer_addr()
                    .map_err(|e| SocksError::BindError(e.to_string()))?;
            ...
            Client::Socks5(inner) => {
                ...
                let target = target_ip.parse::<IpAddr>()
                .map_err(|e| SocksError::BindError(e.to_string()))?;
                ...
                {
                    IpAddr::V4(addr) => {
                        let mut request = vec![0x05, SOCKS_CMD_BIND, 0x00, SOCKS_ADDR_TYPE_IPV4];
                    ...
                    IpAddr::V6(addr) => {
                        let mut request = vec![0x05, SOCKS_CMD_BIND, 0x00, SOCKS_ADDR_TYPE_IPV6];
                        request.extend_from_slice(&addr.octets());
                        request.extend_from_slice(&target_port.to_be_bytes());
                        stream.write_all(&request).await
                        .map_err(|e| SocksError::BindError(e.to_string()))?;
                ...
                let mut bind_response_header = [0; 4];
                stream.read_exact(&mut bind_response_header).await
                .map_err(|e| SocksError::BindError(e.to_string()))?;
                ...
                bind_response[..4].copy_from_slice(&bind_response_header);
