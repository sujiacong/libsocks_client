    ...
    async fn connect(&mut self, target_addr:&str, target_port:u16) -> Result<SocksTcpStream, SocksError> {
        match self {
            Client::Socks4(inner) => {
                ...
                let target = target_str.to_socket_addrs()
                .map_err(|e| SocksError::ConnectionError(e.to_string()))?.next()
                ...
                let mut request = vec![0x04, SOCKS_CMD_CONNECT]; 
                ...
                if response[0] == 0x00 && response[1] == 0x5A
                {
                    ...
                    let bind_addr = format!("{}:{}",bind_ip,bind_port).parse::<SocketAddr>()
                    .map_err(|e| SocksError::ConnectionError(e.to_string()))?;
    
                    Ok(SocksTcpStream::new(stream, bind_addr))

                } else {
                    return Err(SocksError::ConnectionError(format!("SOCKS4 Connect request failed, error {}",translate_socks4_error(response[1]))));
                }
            }
            Client::Socks4a(inner) =>
            {
                ...
                if response[0] == 0x00 && response[1] == 0x5A {
                    ...
                    return Err(SocksError::ConnectionError(format!("SOCKS4A Connect request failed, error {}",translate_socks4_error(response[1]))));
                }
            }
            Client::Socks5(inner) => {
                let target_str = format!("{}:{}", target_addr, target_port);
                ...
                {
                    SocketAddr::V4(addr4) => {
                        let mut request = vec![0x05, SOCKS_CMD_CONNECT, 0x00, SOCKS_ADDR_TYPE_IPV4];
                    ...
                    SocketAddr::V6(addr6) => {
                        let mut request = vec![0x05, SOCKS_CMD_CONNECT, 0x00, SOCKS_ADDR_TYPE_IPV6];
                ...
                stream.read_exact(&mut response[4..4 + addr_len + 2]).await
                .map_err(|e| SocksError::ConnectionError(e.to_string()))?;

                if response[1] != 0x00 {
                    return Err(SocksError::ConnectionError(format!("SOCKS5 Connect request failed, error {}",translate_socks5_error(response[1]))));
                }
        ...
        match self {
            ...
            Client::Socks4a(inner) =>
            {
                ...
                let targetaddr = match target_host {
                    ...
                    TargetHost::Ipv6(_ip) => {
                        return Err(SocksError::ConnectionError("SOCKS4a not support ipv6 ddress".into()))
                    }
                };
                ...
                if response[0] == 0x00 && response[1] == 0x5a {
                    ...
                    return Err(SocksError::ConnectionError("Connection failed!".into()));
                }
            }
            Client::Socks5(inner) => {
                let mut stream = TcpStream::connect(&inner.proxy_addr).await
                ...
                let mut request = vec![0x05, SOCKS_CMD_CONNECT, 0x00, SOCKS_ADDR_TYPE_DOMAIN];
